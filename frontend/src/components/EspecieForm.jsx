...
export default function EspecieForm({ onSaved, onCancel }) {
  ...
  return (
    <div style={{position:'fixed',inset:0,background:'#0009',display:'flex',alignItems:'flex-end',zIndex:100}}>
      <div style={{background:'var(--theme-surface)',borderRadius:'16px 16px 0 0',padding:'1.5rem',width:'100%',maxHeight:'90dvh',overflowY:'auto'}}>
        <h3 style={{color:'var(--theme-primary)',margin:'0 0 1rem'}}>Nueva especie</h3>
        <form onSubmit={submit} style={{display:'flex',flexDirection:'column',gap:10}}>
          <div style={{ display: 'flex', gap: 10 }}>
            ...
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 4 }}>
              <label style={{color:'var(--theme-secondary)',fontSize:'0.78rem',fontWeight:600}}>Categoría *</label>
              <select style={{background:'var(--theme-background)',border:'1px solid var(--theme-border)',borderRadius:8,padding:'0.65rem 0.9rem',color:'var(--theme-text)',fontSize:'0.95rem',outline:'none'}} value={form.categoria} onChange={e => set('categoria', e.target.value)}>
          ...
          <div style={{display:'flex',gap:8,marginTop:4}}>
            <button type="button" style={{flex:1,background:'none',border:'1px solid var(--theme-border)',borderRadius:8,color:'var(--theme-primary)',padding:'0.75rem',fontSize:'0.9rem',cursor:'pointer'}} onClick={onCancel}>Cancelar</button>
            <button type="submit" style={{flex:2,background:'var(--theme-primary)',border:'none',borderRadius:8,color:'#fff',padding:'0.75rem',fontSize:'0.9rem',fontWeight:600,cursor:'pointer'}} disabled={loading}>{loading ? 'Guardando…' : 'Guardar'}</button>
          </div>
        </form>
      </div>
    </div>
...
function Field({ label, value, onChange, italic, textarea, required }) {
  const inputStyle = { background:'var(--theme-background)',border:'1px solid var(--theme-border)',borderRadius:8,padding:'0.65rem 0.9rem',color:'var(--theme-text)',fontSize:'0.95rem',outline:'none', ...(italic ? { fontStyle: 'italic' } : {}) }
  return (
