  ...
  return (
    <div className="page-container" style={{display:'flex',flexDirection:'column',gap:'1rem'}}>
      <div className="page-header" style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
        <h2 className="page-title" style={{color:'var(--bio-primary)',margin:0,fontSize:'1.3rem'}}>Lotes Preparados</h2>
      </div>

      {loading ? <p style={{color:'var(--bio-text-muted)',textAlign:'center',padding:'2rem'}}>Cargando…</p> : (
        <div style={{display:'flex',flexDirection:'column',gap:10}}>
          {lotes.length === 0 ? <p style={{color:'var(--bio-text-muted)',textAlign:'center',padding:'2rem'}}>No hay lotes preparados aún</p> : (
            lotes.map(l => (
              <div key={l.id} className="card">
                ...
                <div style={{display:'flex',gap:15,color:'var(--bio-secondary)',fontSize:'0.82rem',fontWeight:600,margin:'5px 0'}}>
                  <span>Volumen: {l.volumen_l}L</span>
                  <span>Conc: {l.concentracion_x}x</span>
                </div>
                <div style={{fontSize:'0.75rem',color:'var(--bio-text-muted)',marginTop:8}}>
                  <p>Prep: {new Date(l.fecha_preparacion).toLocaleDateString()}</p>
                  <p style={{ color: new Date(l.fecha_expiracion) < new Date() ? 'var(--error)' : 'var(--bio-primary)' }}>
                    Exp: {new Date(l.fecha_expiracion).toLocaleDateString()}
                  </p>
                </div>
                <button style={{background:'var(--bio-background)',border:'1px solid var(--bio-border)',borderRadius:8,color:'var(--bio-primary)',padding:'0.5rem',fontSize:'0.8rem',cursor:'pointer',marginTop:10,width:'100%'}} onClick={() => alert('Imprimiendo etiqueta de reactivo...')}>
